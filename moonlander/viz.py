...
def create_candle_plot(asset, start_date = None, fig_width = 1000, fig_height = 600, colored = True):
    if colored:
    ...
    p.segment(x0='timestamp', y0='high', x1='timestamp', y1='low', source=asset._data_source_increasing, color=INCREASING_COLOR)
    p.segment(x0='timestamp', y0='high', x1='timestamp', y1='low', source=asset._data_source_decreasing, color=DECREASING_COLOR)

    inc_bar = p.vbar(x='timestamp', width=bar_width, top='open', bottom='close', fill_color=INCREASING_COLOR, line_color=INCREASING_COLOR,
        source=asset._data_source_increasing, name="price")
    dec_bar = p.vbar(x='timestamp', width=bar_width, top='open', bottom='close', fill_color=DECREASING_COLOR, line_color=DECREASING_COLOR,
        source=asset._data_source_decreasing, name="price")

    p.add_tools(HoverTool(
        renderers=[inc_bar, dec_bar],
        tooltips=[("Timestamp", "@timestamp{%F}"),
                ("Open", "@open{$0,0.00000}"),
    ...
    y_range_scaling_callback = CustomJS(args={'y_range': p.y_range, 'source': asset.scaling_source}, code='''
    ...
    x_range_callback = CustomJS(args={'x_range': p.x_range, 'interval': asset.provider.timeframe_seconds}, code='''            
    ...
    p2.vbar(x='timestamp', width = bar_width, top='volume', color=DECREASING_COLOR, source=asset._data_source_decreasing, alpha=0.5)
